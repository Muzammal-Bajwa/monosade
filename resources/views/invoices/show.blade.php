...
@section('content')
            ...
            <div class="card-body">
                                ...
                                <tbody>
                                    ...
                                    @endif
                                     ...
                                     <tr>
                                            ...
                                            <td class="text-primary m-r-10 px-2"> {{ $currentWorkspace->priceFormat($invoice->getDueAmounts($invoice->id)) }}</td>
                                        </tr>
                ...
                @php
                $payments =  App\Models\InvoicePayment::where('invoice_id',$invoice->id)->get();
                @endphp
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <div class="form-icon-user">
                                            ...
                                            <input class="form-control currency_input" type="number" id="amount" name="amount" value="{{$invoice->getDueAmounts($invoice->id)}}" min="0" step="0.01" max="{{$invoice->getDueAmounts($invoice->id)}}" placeholder="{{ __('Enter Monthly Price') }}">
                                        </div>
                                    </div>
                                            ...
                                            <form method="post" action="{{ route('client.invoice.payment',[$currentWorkspace->slug,$invoice->id]) }}" class="require-validation" id="payment-form">
                                                ...
                                                <div class="row">
                                                    <div class="form-group col-md-12">
                                                        ...
                                                        <div class="form-icon-user">
                                                            <span class="currency-icon bg-primary">{{ (!empty($currentWorkspace->currency)) ? $currentWorkspace->currency : '$'}}</span>
                                                            <input class="form-control currency_input" required="required" min="0" name="amount" type="number" value="{{$invoice->getDueAmounts($invoice->id)}}" min="0" step="0.01" max="{{$invoice->getDueAmounts($invoice->id)}}" id="amount">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row">
...
@auth('client')
    @if($invoice->getDueAmounts($invoice->id) > 0 && $currentWorkspace->is_stripe_enabled == 1 || $currentWorkspace->is_paypal_enabled == 1  || (isset($paymentSetting['is_paypal_enabled']) && $paymentSetting['is_paypal_enabled'] == 'on') || (isset($paymentSetting['is_paystack_enabled']) && $paymentSetting['is_paystack_enabled'] == 'on') || (isset($paymentSetting['is_flutterwave_enabled']) && $paymentSetting['is_flutterwave_enabled'] == 'on') || (isset($paymentSetting['is_razorpay_enabled']) &&
    $paymentSetting['is_razorpay_enabled'] == 'on') || (isset($paymentSetting['is_mercado_enabled']) && $paymentSetting['is_mercado_enabled'] == 'on') || (isset($paymentSetting['is_paytm_enabled']) && $paymentSetting['is_paytm_enabled'] == 'on') || (isset($paymentSetting['is_mollie_enabled']) && $paymentSetting['is_mollie_enabled'] == 'on') || (isset($paymentSetting['is_skrill_enabled']) && $paymentSetting['is_skrill_enabled'] == 'on') || (isset($paymentSetting['is_coingate_enabled']) && $paymentSetting['is_coingate_enabled'] == 'on'))
        @push('css-page')
            <style>
                #card-element {
